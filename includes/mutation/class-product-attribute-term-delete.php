	...
	public static function mutate_and_get_payload() {
		return static function ( $input, AppContext $context, ResolveInfo $info ) {
			if ( ! $input['attributeId'] ) {
				throw new UserError( __( 'A valid attributeId is required to create a new product attribute term.', 'wp-graphql' ) );
			}
			
			$taxonomy = \wc_attribute_taxonomy_name_by_id( $input['attributeId'] );

			if ( ! $input['id'] ) {
				throw new UserError( __( 'A valid term ID is required to delete a product attribute term.', 'wp-graphql' ) );
			}

			$term = get_term( $input['id'], $taxonomy );
			if ( is_wp_error( $term ) ) {
				throw new UserError( __( 'Invalid term ID.', 'wp-graphql' ) );
			}
			...
			if ( ! $retval ) {
				throw new UserError( __( 'Failed to delete term.', 'wp-graphql' ) );
			}
