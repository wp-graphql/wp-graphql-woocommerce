			...
			switch ( true ) {
				...
				case is_a( $this->source, Product::class ):
					if ( 'upsell' === $this->info->fieldName ) {
						$query_args['post__in'] = ! empty( $this->source->upsell_ids )
							? $this->source->upsell_ids
							: [ '0' ];
					} elseif ( 'crossSell' === $this->info->fieldName ) {
						$query_args['post__in'] = ! empty( $this->source->cross_sell_ids )
							? $this->source->cross_sell_ids
							: [ '0' ];
					} elseif ( 'variations' === $this->info->fieldName ) {
						$query_args['post_parent'] = $this->source->ID;
						$query_args['post__in']    = ! empty( $this->source->children_ids )
						? $this->source->children_ids
						: [ '0' ];
